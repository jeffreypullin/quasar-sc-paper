...
variant_files <- sc_data_files$variant_file
variant_data <- bind_rows(!!!map(variant_files, ~ read_tsv(.x, show_col_types = FALSE)))

# Print all variants with p-value < 5e-20 (for main snp_pvalue column)
variant_data %>%
  filter(snp_x_pseudotime_pvalue < 5e-20) %>%
  arrange(snp_x_pseudotime_pvalue) |>
  select(feature_id, snp_id, maf, snp_pvalue, snp_x_pseudotime_pvalue) |>
...
sc_int_plot_data <- build_qq_plot_data(sc_data_files, "int") |>
  mutate(source = "sc")
#pb_int_plot_data <- pb_data_files |>
#  filter(model == "lm") |>
#  build_qq_plot_data("int") |>
#  mutate(source = "pb")
...
sc_main_plot_data <- build_qq_plot_data(sc_data_files, "main") |>
  mutate(source = "sc")
#pb_main_plot_data <- pb_data_files |>
#  filter(model == "lm") |>
#  build_qq_plot_data("main") |>
#  mutate(source = "pb")
