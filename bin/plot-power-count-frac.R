...
read_egenes <- function(manifest, method) {
  manifest |>
    ...
    mutate(
      ...
      .by = c(cell_type, method, count_frac)
    ) |>
    filter(region_bh < 0.05) |>
    select(cell_type, method, count_frac, feature_id) |>
    distinct()
}
...
grid <- bind_rows(
  pb_manifest |> distinct(cell_type, count_frac) |> mutate(method = "lm"),
  sc_manifest |> distinct(cell_type, count_frac) |> mutate(method = "p_glmm_sc")
) |>
  mutate(
    method_label = factor(
      method_lookup[method],
...
count_data <- grid |>
  left_join(
    egenes |> summarise(n_egene = n(), .by = c(cell_type, method, count_frac)),
    by = c("cell_type", "method", "count_frac")
  ) |>
...
count_frac_breaks <- c(0.1, 0.25, 0.5, 1)
count_frac_labels <- percent(count_frac_breaks, accuracy = NULL, drop0trailing = TRUE)

p_counts <- count_data |>
  ggplot(aes(count_frac, n_egene, colour = method_label, group = method_label)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2) +
  scale_x_continuous(
    breaks = count_frac_breaks,
    labels = count_frac_labels,
...
ggsave(
  "power-count-frac-counts-plot.pdf",
  p_counts,
  width = 12,
  height = 5
)
...
recall_data <- grid |>
  left_join(
    egenes |>
      ...
      summarise(n_recovered = n(), .by = c("cell_type", "method", "count_frac")),
    by = c("cell_type", "method", "count_frac")
  ) |>
...
p_recall <- recall_data |>
  ggplot(aes(count_frac, recall, colour = method_label, group = method_label)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2) +
  scale_x_continuous(
    breaks = count_frac_breaks,
...
ggsave(
  "power-count-frac-recall-plot.pdf",
  p_recall,
  width = 12,
  height = 5
)
