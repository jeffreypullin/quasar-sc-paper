...
plot_perm_count_frac <- function(plot_data, model_label, out_file) {
  count_frac_levels <- sort(unique(plot_data$count_frac))
  count_frac_labels <- percent(count_frac_levels, accuracy = NULL, drop0trailing = TRUE)
  ...
  p <- plot_data |>
    ...
    facet_grid2(
      vars(cell_type), vars(count_frac),
      scales = "free_y",
      independent = "y"
    ) +
    labs(
  ...
  n_frac <- n_distinct(plot_data$count_frac)
  ggsave(
    out_file,
    p,
...
build_sc_plot_data <- function(model_name) {
  sc_data_files |>
    ...
    summarise(file_list = list(variant_file), .by = c(cell_type, count_frac)) |>
    rowwise() |>
...
for (model_name in models) {
  ...
  plot_perm_count_frac(
    plot_data,
    method_lookup[[model_name]],
    sprintf("perm-count-frac-%s-plot.pdf", model_name)
  )
}
