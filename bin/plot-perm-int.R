...
pb_overall_int_plot_data <- pb_data_files |>
  ...
  rowwise() |>
  mutate(qq_data = list(compute_qq_data_pb_int(file_list, prop_file, int_cov, "int"))) |>
  ungroup() |>
  select(-file_list) |>
  unnest(cols = qq_data)
...
ggsave(
  "perm-int-pb-overall-int-plot.pdf",
  pb_overall_int_p,
  width = 12,
  height = 10
)

pb_overall_int_main_plot_data <- pb_data_files |>
  filter(model == "lmm") |>
  filter(cell_frac == 1) |>
  filter(indiv_frac == 1) |>
  summarise(
  ...
  rowwise() |>
  mutate(qq_data = list(compute_qq_data_pb_int(file_list, prop_file, int_cov, "main"))) |>
  ungroup() |>
  select(-file_list) |>
  unnest(cols = qq_data)

pb_overall_int_main_p <- pb_overall_int_main_plot_data |>
  ggplot(aes(log_x_bin_mid, log_y_pvalue,
             ymin = log_lower_ci, ymax = log_upper_ci,
             colour = cell_type)) +
  geom_point(alpha = 0.8) +
...
ggsave(
  "perm-int-pb-overall-int-main-plot.pdf",
  pb_overall_int_main_p,
  width = 12,
  height = 10
)